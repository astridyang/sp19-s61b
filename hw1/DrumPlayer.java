...
public class DrumPlayer {
    ...
    private Drum[] strings;
    private double[] vol;

    public DrumPlayer(InputStream source) {
        try {
    ...
    private void initialize() {
        strings = new Drum[128];
        vol = new double[128];
        for (int i = 0; i < strings.length; i++) {
            strings[i] = new Drum(440.0 * Math.pow(2.0, (i - 69.0) / 12.0));
            vol[i] = 0.0;
        }
    }
